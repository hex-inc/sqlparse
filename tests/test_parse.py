...
def test_parse_casted_params():
    t = (
        sqlparse.parse("foo(DATE '2023-11-14', TIMESTAMP '2023-11-15')")[0]
        .tokens[0]
        .get_parameters()
    )
    assert len(t) == 2
...
def test_typed_array_definition():
    ...
    names = [x.get_name() for x in p.get_sublists() if isinstance(x, sql.Identifier)]
    assert names == ["x", "y", "z"]
...
def test_parenthesis():
    ...
    assert list(map(lambda t: t.ttype, tokens)) == [
        T.Punctuation,
        T.Newline,
        T.Newline,
        T.Number.Integer,
